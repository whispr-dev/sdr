...
# ------------------- TX worker (single shared device) -------------------
...
class TxWorker(threading.Thread):
    ...
    def run(self):
        ...
        try:
            if a.tx_mode in ("tone","pulse"):
                n = 4096
                t = np.arange(n, dtype=np.float32) / float(a.tx_rate)
                ph = 2*np.pi*float(a.tone_hz)*t
                on_wave = (a.amplitude*(np.cos(ph)+1j*np.sin(ph))).astype(np.complex64)
                off_wave = np.zeros(n, dtype=np.complex64)  # keep stream active but RF off

                t_end = time.time() + a.seconds if a.seconds>0 else None
                period = float(a.pulse_period)
                duty = float(a.pulse_duty)
                while not self.stop_evt.is_set() and (t_end is None or time.time()<t_end):
                    if a.tx_mode == "pulse":
                        now = time.time()
                        phase = (now % period) / period
                        buf = on_wave if phase < duty else off_wave
                    else:
                        buf = on_wave
                    if write_stream_compat(self.sdr, self.st, buf) <= 0: continue
...
class RxViewer:
    def __init__(self, sdr, ch, args, stop_evt: threading.Event):
        self.sdr, self.ch, self.a, self.stop_evt = sdr, ch, args, stop_evt
        ...
        self.fig, (self.ax_psd, self.ax_wf) = plt.subplots(2,1, figsize=(10,6), gridspec_kw=dict(height_ratios=[1,2]))
        self.fig.canvas.mpl_disconnect(self.fig.canvas.manager.key_press_handler_id)  # disable default shortcuts
        ...
        self.update_title()
        self.im = self.ax_wf.imshow(self.wf, aspect="auto", origin="lower", vmin=-100, vmax=-40, interpolation="nearest")
        ...
        self.mute_bins = []; self.recompute_mute_bins()
        self.fig.canvas.mpl_connect("key_press_event", self.on_key)
        signal.signal(signal.SIGINT, self.sigint)

    def update_title(self):
        self.ax_psd.set_title(f"RX @ {self.center/1e6:.3f} MHz  fs={self.rate/1e6:.2f} Msps  gain {self.a.gain:.1f} dB  avg {self.alpha:.2f}")
    ...
    def on_key(self, ev):
        ...
        if ev.key == "left":
            ...
            self.a.gain = min(70.0, self.a.gain + 2.0); self.sdr.setGain(SOAPY_SDR_RX, self.ch, self.a.gain)
        elif ev.key == "down":
            self.a.gain = max(0.0, self.a.gain - 2.0); self.sdr.setGain(SOAPY_SDR_RX, self.ch, self.a.gain)
    ...
    def sigint(self,*_):
        self.stop_evt.set(); plt.close("all")
        try: self.sdr.deactivateStream(self.st); self.sdr.closeStream(self.st)
        except Exception: pass
        sys.exit(0)
...
def main():
    ap = argparse.ArgumentParser(description="LimeSDR-Mini v2 full-duplex: TX (tone/pulse/file/off) + live RX waterfall (single device).")
    ...
    # TX
    ap.add_argument("--tx-mode", choices=["off","tone","pulse","file"], default="pulse")
    ap.add_argument("--tx-freq", type=float, help="Defaults to --freq")
    ap.add_argument("--tx-rate", type=float, default=2e6)
    ap.add_argument("--tx-bw", type=float, default=None)
    ap.add_argument("--tx-gain", type=float, default=35.0)
    ap.add_argument("--tx-path", default="BAND2")
    ap.add_argument("--tx-antenna", default=None)
    ap.add_argument("--pad", type=float)
    ap.add_argument("--iamp", type=float)
    ap.add_argument("--tone-hz", type=float, default=100e3)
    ap.add_argument("--amplitude", type=float, default=0.2)
    ap.add_argument("--iq")
    # pulse params (for --tx-mode pulse)
    ap.add_argument("--pulse-period", type=float, default=1.0, help="seconds, total on+off")
    ap.add_argument("--pulse-duty", type=float, default=0.5, help="0..1, on-time fraction")
    # Plot & control
    ...
    ap.add_argument("--marker", action="append", type=float)
    # misc
    ...
    tx_thr = TxWorker(sdr, tx_ch, args, stop_evt); tx_thr.start()
    viewer = RxViewer(sdr, rx_ch, args, stop_evt); viewer.loop()
